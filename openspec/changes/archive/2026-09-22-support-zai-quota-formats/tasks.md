...
- [x] 7.1 TEST: ввести в `providerDetection.test.ts` фиксированную константу `NOW`
      и передавать её в каждый новый вызов парсера: действующие фикстуры опираются
      на люфт системных часов.
- [x] 7.2 TEST: живая фикстура кредитного тарифа — утилизация около 0,596 и 0,3406,
...
- [x] 7.3 TEST: живая фикстура токенного тарифа — пятичасовая утилизация 0 с запасным
      горизонтом около 18 000 с, недельная утилизация 1, статус `denied`, тарификация
      токенная, величин кредитов нет.
- [x] 7.4 TEST: ведущий `TIME_LIMIT` на 99 % не занимает окна квоты и не поднимает
      предупреждения.
- [x] 7.5 TEST: величины без `percentage` дают выведенное отношение, а не ноль; величины
      без `remaining` всё равно отдают израсходованное и полное.
- [x] 7.6 TEST: ответы без единицы периода — разбор по порядку, недельное окно есть,
...
- [x] 7.7 TEST: смешанный ответ (одна запись с единицей, другая без), недокументированная
      единица 2 и окно квоты с месячным периодом, не занимающее слот.
- [x] 7.8 TEST: времена сброса в прошлом и на 500 суток вперёд откатываются к длине окна,
      а недельный откат равен семи суткам, а не одним.
- [x] 7.9 TEST: работа с конвертом — код 1000 повторяет попытку и затем бросает, код 500
...
- [x] 7.10 TEST: враждебный вход (null, строка, число, `limits` не-массив, элементы null
      и не-объекты, слишком длинный массив) даёт нули без исключения.
- [x] 7.11 TEST: отказ аутентификации подавляет следующий запрос до истечения TTL, а сетевой
      отказ — нет.
- [x] 7.12 TEST: шесть действующих тестов z.ai остаются зелёными. Их фикстуры
...
- [x] 8.2 TEST: новый `src/test/suite/anthropicRateLimit.test.ts` — разбор заголовков,
...
- [x] 8.3 TEST: специфичные для z.ai поля остаются пустыми в результатах Anthropic.
- [x] 8.4 TEST: круговой прогон через кэш v4 сохраняет `has7dLimit` ложным для тарифа без
...
- [x] 8.5 TEST: запись версии 3 принимается на чтение и переписывается версией 4; запись
      с утилизацией вне диапазона или неизвестным статусом по-прежнему отвергается в любой
      версии.
- [x] 8.6 TEST: дашборд скрывает блок величин и значок уровня, когда поля отсутствуют, —
      и для Anthropic, и для токенного тарифа (`panel.test.ts`).
- [x] 8.7 Обновить версионные фикстуры, которые изменение неизбежно обесценивает:
...
- [x] 8.8 Убедиться, что наборы statusBar, notificationDecision, prediction и locale
...
- [x] 9.1 Добавить в `docs/DATA.md` раздел о квоте z.ai: оба поколения ответа, перевёрнутое
      именование `usage` и `currentValue`, и почему порядок важнее времени сброса при
      определении окон.
- [x] 9.2 Добавить запись в `## [Unreleased]` в `CHANGELOG.md`: на прежних тарифах z.ai
...
- [x] 9.3 Убедиться, что `.env` игнорируется git и токены для ручной проверки не могут
      попасть в коммит.

## 10. Проверка

- [x] 10.1 `npm run lint` и `npm test` проходят. Прогнано в CI: Linux — 222 теста, Windows —
      220, оба без падений. Локально и на macOS не проверялось, см. примечание ниже.
- [x] 10.2 Только для владельца токенов, остальную работу не блокирует: прогнать оба живых
      токена через новый парсер и сверить с панелью подписки z.ai. Снятые для 7.2 и 7.3
      ответы — долговечная форма этой проверки.
- [x] 10.3 Extension Dev Host на кредитном тарифе: настоящие проценты, величины кредитов,
...
- [x] 10.4 `openspec validate support-zai-quota-formats --strict` проходит.
