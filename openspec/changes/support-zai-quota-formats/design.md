...
которому z.ai выставляет счёт (месяц), и достаточно близко, чтобы поймать промашку
вышестоящего API в сторону завышения: значение примерно в тысячу раз больше ожидаемого,
например микросекунды, прочитанные как миллисекунды, уносит на десятки тысяч лет вперёд.
Обратная промашка, секунды вместо миллисекунд, даёт значение примерно в тысячу раз меньше:
оно попадает в 1970 год, то есть в прошлое, где его ловит откат для сброса в прошлом.
Число живёт здесь, чтобы требование и код не разъехались без записанной причины.
...
Так же подавляется и отказ из-за нечитаемой формы ответа (D6). Это второй класс, который
повтор не лечит: payload, переставший разбираться, сам собой разбираться не начнёт, а до
этого изменения он давал закэшированный ноль и шторма не создавал — значит шторм здесь
тоже вносим мы. Сетевые и вышестоящие отказы намеренно **не** подавляются: они преходящи
и повтора стоят, тогда как отозванный ключ через пять минут останется отозванным.

Причина подавления хранится рядом со временем, потому что действовать пользователю нужно
только в одном из двух случаев: отвергнутый ключ называется в интерфейсе, дрейф формата —
нет, иначе мы обвинили бы исправный ключ.
